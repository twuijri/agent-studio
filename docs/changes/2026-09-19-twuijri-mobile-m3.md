...
## إضافة: إصلاح تشغيل اختبارات iOS على CI (2026-09-19)
- `ac0f78eb`: اختبار مصافحة السوكت كان يقارن `url.path` بـ `"/socket.io/"`،
  ونسخة Foundation على محاكي CI تحذف الشرطة الأخيرة، فسقط الاختبار رغم صحة
  الرابط. صار المقارنة على النص الكامل للرابط.
- `8b9cd182` ثم `8e6fd677`: طباعة رسائل الإخفاق من حزمة النتائج
  (`-resultBundlePath` + `scripts/ci/xcresult-failures.py`) لأن Xcode الحالي لا
  يكتب نص التوكيد في stdout؛ نُقل السكربت إلى ملف بعد أن كسر التضمين المباشر
  تنسيق YAML.
- `299f06c0`: الخطوة كانت تنتهي بخطأ صامت بعد نجاح الاختبارات؛ صدفة المشغّل
  `bash -e` مع `set -o pipefail` تجعل `grep` الأخير (سطر الملخّص) يُسقط المهمة
  حين تتغيّر صياغة Xcode. صار الملخّص إعلاميًا، وحُصّن إسناد وجهة المحاكي ضد
  SIGPIPE من `grep -m1`.
