...
describe('agent bridge manager command resolution', () => {
  ...
  it('prefers the Core Hub bundled runtime env over a user-installed Hermes command', async () => {
  ...
  it('injects Web UI OpenRouter attribution into the bridge process env by default', async () => {
    ...
    expect(env.HERMES_OPENROUTER_APP_REFERER).toBe('https://github.com/twuijri/agent-studio')
    expect(env.HERMES_OPENROUTER_APP_TITLE).toBe('Core Hub')
