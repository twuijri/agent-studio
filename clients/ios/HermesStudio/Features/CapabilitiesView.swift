...
/// Skills of one agent: Hermes by default, or a coding agent through the
/// `target` query (`CodingAgentSkillsPanel.vue`). Reached from the agent's
/// own screen only; there is no app-level skills entry.
struct SkillsView: View {
    ...
    var body: some View {
            ...
            ForEach(categories, id: \.self) { category in
                Section(category.capitalized) {
                    ForEach(filtered.filter { $0.category == category }) { skill in
                        NavigationLink { SkillEditorView(skill: skill, target: target) { await load() } } label: {
        ...
        }.listStyle(.insetGrouped).navigationTitle(NavDestination.skills.title).navigationBarTitleDisplayMode(.inline).overlay { if loading { ProgressView() } }.refreshable { await load() }.task(id: store.selectedProfile) { await load() }
    }
    ...
    private func load() async { loading = true; do { async let skillsRequest = store.api.skills(profile: store.selectedProfile, target: target); async let approvalsRequest = store.api.pendingSkillWrites(profile: store.selectedProfile); skills = try await skillsRequest; pendingWrites = try await approvalsRequest } catch { store.errorMessage = error.localizedDescription }; loading = false }
    ...
    private func toggle(_ skill: SkillItem) async { do { try await store.api.toggleSkill(skill, profile: store.selectedProfile, target: target); await load() } catch { store.errorMessage = error.localizedDescription } }
    private func pin(_ skill: SkillItem) async { do { try await store.api.pinSkill(skill, profile: store.selectedProfile, target: target); await load() } catch { store.errorMessage = error.localizedDescription } }
}
...
private struct SkillEditorView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack { StatusPill(text: skill.category.capitalized, color: .blue); Spacer(); Toggle("Enabled", isOn: Binding(get: { skill.enabled }, set: { _ in Task { try? await store.api.toggleSkill(skill, profile: store.selectedProfile, target: target); skill.enabled.toggle() } })).labelsHidden() }.padding()
            ...
            .toolbar { ToolbarItemGroup(placement: .topBarTrailing) { Button { skill.pinned.toggle(); Task { try? await store.api.pinSkill(skill, profile: store.selectedProfile, target: target) } } label: { Image(systemName: skill.pinned ? "pin.fill" : "pin") }; Button("Save") { Task { await save() } }.disabled(saving) } }
            .task { do { skill = try await store.api.skill(category: skill.category, name: skill.name, profile: store.selectedProfile, target: target) } catch { store.errorMessage = error.localizedDescription }; loaded = true }
            .safeAreaInset(edge: .bottom) { Button("Delete skill", role: .destructive) { deleteConfirm = true }.font(.footnote).padding(8) }
            .alert("Delete skill?", isPresented: $deleteConfirm) { Button("Delete", role: .destructive) { Task { try? await store.api.deleteSkill(skill, profile: store.selectedProfile, target: target); await onChange(); dismiss() } }; Button("Cancel", role: .cancel) {} }
    }
    private func save() async { saving = true; do { try await store.api.saveSkill(skill, profile: store.selectedProfile, target: target); await onChange(); store.notify(String(localized: "Skill saved")) } catch { store.errorMessage = error.localizedDescription }; saving = false }
}
...
struct PluginsView: View {
    ...
    var body: some View {
        ...
        }.listStyle(.insetGrouped).navigationTitle(NavDestination.plugins.title).navigationBarTitleDisplayMode(.inline).overlay { if loading { ProgressView() } }.refreshable { await load() }.task { await load() }
    }
...
struct MCPView: View {
    ...
    var body: some View {
        Group { if !loading && servers.isEmpty { EmptyState(icon: "server.rack", title: "No MCP servers", detail: "Connect tools using a local command or remote URL.") } else { List(servers) { server in Button { editing = server } label: { HStack(spacing: 12) { Image(systemName: server.url.isEmpty ? "terminal.fill" : "network").foregroundStyle(.cyan).frame(width: 42, height: 42).background(.cyan.opacity(0.12), in: RoundedRectangle(cornerRadius: 12)); VStack(alignment: .leading, spacing: 4) { Text(server.name).font(.headline).foregroundStyle(.primary); Text(server.url.nilIfEmpty ?? ([server.command] + server.arguments).joined(separator: " ")).font(.caption.monospaced()).foregroundStyle(.secondary).lineLimit(2); if !server.tools.isEmpty { Text("\(server.tools.count) tools").font(.caption2).foregroundStyle(.secondary) } }; Spacer(); StatusPill(text: server.enabled ? String(localized: "On") : String(localized: "Off"), color: server.enabled ? .green : .gray) }.padding(.vertical, 4) }.buttonStyle(.plain).swipeActions { Button(role: .destructive) { Task { await delete(server) } } label: { Label("Delete", systemImage: "trash") } } }.listStyle(.insetGrouped) } }
            .navigationTitle(NavDestination.mcp.title).navigationBarTitleDisplayMode(.inline).toolbar { ToolbarItemGroup(placement: .topBarTrailing) { Button { Task { await load() } } label: { Image(systemName: "arrow.clockwise") }; Button { creating = true } label: { Image(systemName: "plus") } } }.overlay { if loading { ProgressView() } }.refreshable { await load() }.task { await load() }.sheet(isPresented: $creating) { MCPEditor(server: nil) { await load() } }.sheet(item: $editing) { MCPEditor(server: $0) { await load() } }
    }
...
struct ProviderAuthView: View { @EnvironmentObject var store: AppStore; let provider: String; @State var authenticated = false; @State var sessionID = ""; @State var userCode = ""; @State var url = ""; @State var submitCode = ""; @State var status = ""
    ...
    private func poll() async { do { let result = try await store.api.pollProviderAuth(provider, sessionID: sessionID); status = result.string("status"); authenticated = status == "approved" } catch { store.errorMessage = error.localizedDescription } }
    private func submit() async { do { let result = try await store.api.submitProviderAuth(provider, sessionID: sessionID, code: submitCode); status = result.string("status"); authenticated = status == "approved" } catch { store.errorMessage = error.localizedDescription } }
}

struct EkkoSkillsView: View {
    @EnvironmentObject private var store: AppStore
    @State private var items: [EkkoSkillItem] = []; @State private var search = ""; @State private var selected: EkkoSkillItem?; @State private var importing = false; @State private var creating = false; @State private var externalDirs = false
    var body: some View { List { SearchBar(text: $search); ForEach(items) { item in Button { Task { selected = try? await store.api.ekkoSkill(item.name) } } label: { HStack { VStack(alignment: .leading) { Text(item.name).font(.headline).foregroundStyle(.primary); Text(item.description).font(.caption).foregroundStyle(.secondary).lineLimit(2) }; Spacer(); Toggle("", isOn: Binding(get: { item.enabled }, set: { value in Task { try? await store.api.setEkkoSkill(item.name, enabled: value); await load() } } )).labelsHidden() } }.swipeActions { Button(role: .destructive) { Task { try? await store.api.deleteEkkoSkill(item.name); await load() } } label: { Label("Delete", systemImage: "trash") } } } }.navigationTitle(NavDestination.skills.title).navigationBarTitleDisplayMode(.inline).task(id: search) { try? await Task.sleep(for: .milliseconds(250)); await load() }.sheet(item: $selected) { EkkoSkillDetailView(item: $0) { await load() } }.sheet(isPresented: $creating) { EkkoSkillCreateView { await load() } }.sheet(isPresented: $externalDirs) { EkkoExternalDirsView() }.toolbar { Menu { Button("Create skill") { creating = true }; Button("Import skill") { importing = true }; Button("External directories") { externalDirs = true } } label: { Image(systemName: "plus") } }.fileImporter(isPresented: $importing, allowedContentTypes: [.data]) { result in if case let .success(url) = result { Task { await importSkill(url) } } } }
...
private struct EkkoSkillDetailView: View { @EnvironmentObject var store: AppStore; @Environment(\.dismiss) var dismiss; @State var item: EkkoSkillItem; @State var files: [JSON] = []; let saved: () async -> Void
    @State private var fileContent = ""; @State private var fileTitle = ""; @State private var showingFile = false
    var body: some View { NavigationStack { Form { Section("Skill") { LabeledContent("Category", value: item.category); LabeledContent("Source", value: item.source); TextEditor(text: $item.content).frame(minHeight: 240).font(.body.monospaced()) }; if !files.isEmpty { Section("Files") { ForEach(Array(files.enumerated()), id: \.offset) { _, file in Button { Task { fileTitle = file.string("path", "name"); fileContent = (try? await store.api.ekkoSkillFile(item.name, path: fileTitle)) ?? ""; showingFile = true } } label: { Label(file.string("path", "name"), systemImage: file.bool("directory") ? "folder" : "doc") } } } } }.navigationTitle(item.name).toolbar { ToolbarItem(placement: .cancellationAction) { Button("Cancel") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("Save") { Task { try? await store.api.saveEkkoSkill(item); await saved(); dismiss() } } } }.task { files = (try? await store.api.ekkoSkillFiles(item.name)) ?? [] }.sheet(isPresented: $showingFile) { NavigationStack { ScrollView { Text(fileContent).font(.body.monospaced()).textSelection(.enabled).frame(maxWidth: .infinity, alignment: .leading).padding() }.navigationTitle(fileTitle).toolbar { Button("Done") { showingFile = false } } } } } }
}

private struct EkkoSkillCreateView: View { @EnvironmentObject var store: AppStore; @Environment(\.dismiss) var dismiss; @State var name = ""; @State var category = "workspace"; @State var content = "---\ndescription: \n---\n"; let saved: () async -> Void
    var body: some View { NavigationStack { Form { TextField("Name", text: $name); TextField("Category", text: $category); TextEditor(text: $content).frame(minHeight: 250).font(.body.monospaced()) }.navigationTitle("Create skill").toolbar { ToolbarItem(placement: .cancellationAction) { Button("Cancel") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("Create") { Task { do { try await store.api.createEkkoSkill(name: name, content: content, category: category); await saved(); dismiss() } catch { store.errorMessage = error.localizedDescription } } }.disabled(name.isEmpty) } } } }
}
private struct EkkoExternalDirsView: View { @EnvironmentObject var store: AppStore; @Environment(\.dismiss) var dismiss; @State var text = ""
    var body: some View { NavigationStack { Form { Section { TextField("One directory per line", text: $text, axis: .vertical).lineLimit(6...16) } footer: { Text("Ekko discovers skills from these server-side directories.") } }.navigationTitle("External directories").toolbar { ToolbarItem(placement: .cancellationAction) { Button("Cancel") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("Save") { Task { try? await store.api.saveEkkoExternalDirectories(text.split(separator: "\n").map(String.init)); dismiss() } } } }.task { text = ((try? await store.api.ekkoExternalDirectories()) ?? []).joined(separator: "\n") } } }
}
