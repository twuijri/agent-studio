...
struct WorkflowsView: View {
    ...
    private func cancelImport() async { await store.attempt({ try await store.api.cancelWorkflowImport(token: importToken, profile: store.selectedProfile) }); importToken = "" }
}
