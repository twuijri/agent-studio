...
function windowsCliForwarder(runtimePlatform: string, runtimeVersion: string): string {
  ...
  return [
    ...
    "if(!fs.existsSync(python)){console.error('Core Hub Python runtime not found at '+python);console.error('Open Core Hub once to finish runtime setup, then retry ekko-studio cli.');process.exit(127)}",
...
export function createPowerShellShimContent(
  ...
  return [
    ...
    "  [Console]::Out.WriteLine('  (no command)       Open Core Hub desktop app')",
    ...
    "      [Console]::Error.WriteLine('Core Hub executable not found at ' + $App)",
    '      exit 127',
    '    }',
    ...
    "      [Console]::Error.WriteLine('Core Hub Node runtime not found at ' + $Node)",
    "      [Console]::Error.WriteLine('Open Core Hub once to finish runtime setup, then retry ekko-studio cli.')",
    '      exit 127',
    '    }',
    ...
    '    if (!(Test-Path -LiteralPath $Node -PathType Leaf)) {',
    "      [Console]::Error.WriteLine('Core Hub Node runtime not found at ' + $Node)",
    "      [Console]::Error.WriteLine('Open Core Hub once to finish runtime setup, then retry ekko-studio web.')",
    '      exit 127',
    '    }',
    ...
    '  }',
    '  default {',
    "    [Console]::Error.WriteLine('Unknown Core Hub command: ' + $Command)",
...
export function createShimContent(
  ...
  return [
    ...
    '',
    'Commands:',
    '  (no command)       Open Core Hub desktop app',
    ...
    '}',
    'if [ ! -x "$APP" ]; then',
    '  echo "Core Hub executable not found at $APP" >&2',
    '  exit 127',
    'fi',
    ...
    '    shift',
    '    if [ ! -x "$NODE" ]; then',
    '      echo "Core Hub Node runtime not found at $NODE" >&2',
    '      echo "Open Core Hub once to finish runtime setup, then retry ekko-studio web." >&2',
    '      exit 127',
    '    fi',
    ...
    '    ;;',
    '  *)',
    '    echo "Unknown Core Hub command: $1" >&2',
...
export function createMcpShimContent(
  ...
  return [
    ...
    'if [ ! -x "$NODE" ]; then',
    '  echo "Core Hub Node runtime not found at $NODE" >&2',
    '  echo "Open Core Hub once to finish runtime setup, then retry ekko-studio-mcp." >&2',
    '  exit 127',
    'fi',
    'if [ ! -f "$SCRIPT" ]; then',
    '  echo "Core Hub MCP script not found at $SCRIPT" >&2',
    '  exit 127',
    'fi',
...
export function createMcpPowerShellShimContent(
  ...
  return [
    ...
    'if (!(Test-Path -LiteralPath $Node -PathType Leaf)) {',
    "  [Console]::Error.WriteLine('Core Hub Node runtime not found at ' + $Node)",
    "  [Console]::Error.WriteLine('Open Core Hub once to finish runtime setup, then retry ekko-studio-mcp.')",
    '  exit 127',
    '}',
    'if (!(Test-Path -LiteralPath $Script -PathType Leaf)) {',
    "  [Console]::Error.WriteLine('Core Hub MCP script not found at ' + $Script)",
    '  exit 127',
    '}',
...
export async function installHermesStudioCliShim(options: CliShimInstallOptions = {}): Promise<CliShimInstallResult> {
  ...
  return {
    ...
    status,
    pathUpdated,
    reason: status === 'skipped' ? 'existing ekko-studio shim is not managed by Core Hub' : undefined,
  }
}
...
export async function installHermesStudioMcpShim(options: McpShimInstallOptions = {}): Promise<CliShimInstallResult> {
  ...
  return {
    ...
    status,
    pathUpdated,
    reason: status === 'skipped' ? 'existing ekko-studio-mcp shim is not managed by Core Hub' : undefined,
  }
}
