...
type TranslationKey =
  ...
  | 'tray.releaseNotices'
  | 'tray.newVersion'
  | 'release.availableTitle'
  | 'release.availableMessage'
  | 'release.availableDetail'
  | 'release.notificationBody'
  | 'release.open'
  | 'release.later'
  | 'release.skip'
  | 'release.upToDateMessage'
  | 'release.failedMessage'
  | 'tray.resetLogin'
  | 'tray.openAtLogin'
  | 'tray.quit'
...
const translations: Record<Exclude<DesktopLocale, 'ar'>, Record<TranslationKey, string>> = {
  en: {
    ...
    'tray.releaseNotices': 'Notify about new versions',
    'tray.newVersion': 'Download version {version}…',
    'release.availableTitle': 'A new version of Core Hub is available',
    'release.availableMessage': 'Version {version} is available (you have {current}).',
    'release.availableDetail': 'Updates are never installed automatically. Open the release page, download the build for your computer and install it over the current version; your settings and server link stay.',
    'release.notificationBody': 'Core Hub {version} is available. Click to open the download page.',
    'release.open': 'Open download page',
    'release.later': 'Later',
    'release.skip': 'Skip this version',
    'release.upToDateMessage': 'Core Hub {current} is the latest version.',
    'release.failedMessage': 'Could not check for new versions: {error}',
  ...
  zh: {
    ...
    'tray.releaseNotices': '有新版本时通知',
    'tray.newVersion': '下载版本 {version}…',
    'release.availableTitle': 'Core Hub 有新版本可用',
    'release.availableMessage': '版本 {version} 已发布（当前为 {current}）。',
    'release.availableDetail': '更新不会自动安装。请打开发布页面，下载适合本机的安装包并覆盖安装；设置和服务器连接会保留。',
    'release.notificationBody': 'Core Hub {version} 已发布。点击打开下载页面。',
    'release.open': '打开下载页面',
    'release.later': '稍后',
    'release.skip': '跳过此版本',
    'release.upToDateMessage': 'Core Hub {current} 已是最新版本。',
    'release.failedMessage': '无法检查新版本：{error}',
  ...
  'zh-TW': {
    ...
    'tray.releaseNotices': '有新版本時通知',
    'tray.newVersion': '下載版本 {version}…',
    'release.availableTitle': 'Core Hub 有新版本可用',
    'release.availableMessage': '版本 {version} 已發佈（目前為 {current}）。',
    'release.availableDetail': '更新不會自動安裝。請開啟發佈頁面，下載適合本機的安裝檔並覆蓋安裝；設定與伺服器連結會保留。',
    'release.notificationBody': 'Core Hub {version} 已發佈。點選以開啟下載頁面。',
    'release.open': '開啟下載頁面',
    'release.later': '稍後',
    'release.skip': '略過此版本',
    'release.upToDateMessage': 'Core Hub {current} 已是最新版本。',
    'release.failedMessage': '無法檢查新版本：{error}',
  ...
  ja: {
    ...
    'tray.releaseNotices': '新しいバージョンを通知する',
    'tray.newVersion': 'バージョン {version} をダウンロード…',
    'release.availableTitle': 'Core Hub の新しいバージョンがあります',
    'release.availableMessage': 'バージョン {version} が利用できます（現在は {current}）。',
    'release.availableDetail': '更新は自動では行われません。リリースページを開いてお使いのコンピューター向けのビルドをダウンロードし、現在のバージョンに上書きインストールしてください。設定とサーバー接続は保持されます。',
    'release.notificationBody': 'Core Hub {version} が利用できます。クリックしてダウンロードページを開きます。',
    'release.open': 'ダウンロードページを開く',
    'release.later': '後で',
    'release.skip': 'このバージョンをスキップ',
    'release.upToDateMessage': 'Core Hub {current} は最新バージョンです。',
    'release.failedMessage': '新しいバージョンを確認できませんでした: {error}',
    'tray.resetLogin': 'Reset Login',
  ...
  ko: {
    ...
    'tray.releaseNotices': '새 버전 알림',
    'tray.newVersion': '버전 {version} 다운로드…',
    'release.availableTitle': 'Core Hub의 새 버전이 있습니다',
    'release.availableMessage': '버전 {version}을(를) 사용할 수 있습니다(현재 {current}).',
    'release.availableDetail': '업데이트는 자동으로 설치되지 않습니다. 릴리스 페이지를 열어 이 컴퓨터용 빌드를 내려받은 뒤 현재 버전 위에 설치하세요. 설정과 서버 연결은 유지됩니다.',
    'release.notificationBody': 'Core Hub {version}을(를) 사용할 수 있습니다. 클릭하면 다운로드 페이지가 열립니다.',
    'release.open': '다운로드 페이지 열기',
    'release.later': '나중에',
    'release.skip': '이 버전 건너뛰기',
    'release.upToDateMessage': 'Core Hub {current}이(가) 최신 버전입니다.',
    'release.failedMessage': '새 버전을 확인할 수 없습니다: {error}',
    'tray.resetLogin': 'Reset Login',
  ...
  fr: {
    ...
    'tray.releaseNotices': 'Signaler les nouvelles versions',
    'tray.newVersion': 'Télécharger la version {version}…',
    'release.availableTitle': 'Une nouvelle version de Core Hub est disponible',
    'release.availableMessage': 'La version {version} est disponible (vous avez la {current}).',
    'release.availableDetail': 'Les mises à jour ne sont jamais installées automatiquement. Ouvrez la page de la version, téléchargez la build pour votre ordinateur et installez-la par-dessus la version actuelle ; vos réglages et le lien au serveur sont conservés.',
    'release.notificationBody': 'Core Hub {version} est disponible. Cliquez pour ouvrir la page de téléchargement.',
    'release.open': 'Ouvrir la page de téléchargement',
    'release.later': 'Plus tard',
    'release.skip': 'Ignorer cette version',
    'release.upToDateMessage': 'Core Hub {current} est la dernière version.',
    'release.failedMessage': 'Impossible de vérifier les nouvelles versions : {error}',
    'tray.resetLogin': 'Reset Login',
  ...
  es: {
    ...
    'tray.releaseNotices': 'Avisar de versiones nuevas',
    'tray.newVersion': 'Descargar la versión {version}…',
    'release.availableTitle': 'Hay una nueva versión de Core Hub',
    'release.availableMessage': 'La versión {version} está disponible (tienes la {current}).',
    'release.availableDetail': 'Las actualizaciones nunca se instalan automáticamente. Abre la página de la versión, descarga la compilación para tu equipo e instálala sobre la versión actual; tus ajustes y el enlace al servidor se conservan.',
    'release.notificationBody': 'Core Hub {version} está disponible. Haz clic para abrir la página de descarga.',
    'release.open': 'Abrir página de descarga',
    'release.later': 'Más tarde',
    'release.skip': 'Omitir esta versión',
    'release.upToDateMessage': 'Core Hub {current} es la última versión.',
    'release.failedMessage': 'No se pudo comprobar si hay versiones nuevas: {error}',
    'tray.resetLogin': 'Reset Login',
  ...
  de: {
    ...
    'tray.releaseNotices': 'Über neue Versionen benachrichtigen',
    'tray.newVersion': 'Version {version} herunterladen…',
    'release.availableTitle': 'Eine neue Version von Core Hub ist verfügbar',
    'release.availableMessage': 'Version {version} ist verfügbar (installiert ist {current}).',
    'release.availableDetail': 'Updates werden nie automatisch installiert. Öffne die Release-Seite, lade den Build für deinen Computer herunter und installiere ihn über die aktuelle Version; Einstellungen und Serververbindung bleiben erhalten.',
    'release.notificationBody': 'Core Hub {version} ist verfügbar. Klicken, um die Download-Seite zu öffnen.',
    'release.open': 'Download-Seite öffnen',
    'release.later': 'Später',
    'release.skip': 'Diese Version überspringen',
    'release.upToDateMessage': 'Core Hub {current} ist die neueste Version.',
    'release.failedMessage': 'Neue Versionen konnten nicht geprüft werden: {error}',
    'tray.resetLogin': 'Reset Login',
  ...
  pt: {
    ...
    'tray.releaseNotices': 'Avisar sobre novas versões',
    'tray.newVersion': 'Baixar a versão {version}…',
    'release.availableTitle': 'Há uma nova versão do Core Hub',
    'release.availableMessage': 'A versão {version} está disponível (você tem a {current}).',
    'release.availableDetail': 'As atualizações nunca são instaladas automaticamente. Abra a página da versão, baixe a build para o seu computador e instale-a sobre a versão atual; suas configurações e o vínculo com o servidor são mantidos.',
    'release.notificationBody': 'Core Hub {version} está disponível. Clique para abrir a página de download.',
    'release.open': 'Abrir página de download',
    'release.later': 'Mais tarde',
    'release.skip': 'Pular esta versão',
    'release.upToDateMessage': 'Core Hub {current} é a versão mais recente.',
    'release.failedMessage': 'Não foi possível verificar novas versões: {error}',
    'tray.resetLogin': 'Reset Login',
...
const arabicTranslations: Partial<Record<TranslationKey, string>> = {
  'tray.releaseNotices': 'التنبيه عند صدور نسخة جديدة',
  'tray.newVersion': 'تنزيل النسخة {version}…',
  'release.availableTitle': 'صدرت نسخة جديدة من كور هب',
  'release.availableMessage': 'النسخة {version} متوفرة (لديك {current}).',
  'release.availableDetail': 'التحديثات لا تُثبَّت تلقائياً أبداً. افتح صفحة الإصدار ونزّل نسخة جهازك وثبّتها فوق النسخة الحالية؛ إعداداتك وربطك بالسيرفر يبقيان كما هما.',
  'release.notificationBody': 'صدرت نسخة كور هب {version}. اضغط لفتح صفحة التنزيل.',
  'release.open': 'فتح صفحة التنزيل',
  'release.later': 'لاحقاً',
  'release.skip': 'تخطّي هذه النسخة',
  'release.upToDateMessage': 'كور هب {current} هي أحدث نسخة.',
  'release.failedMessage': 'تعذّر التحقق من النسخ الجديدة: {error}',
