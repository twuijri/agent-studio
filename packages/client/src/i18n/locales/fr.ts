...
jobTriggered: 'Job declenche',
  ...
  workflow: {
    ...
    node: {
      approvalRequired: 'Approbation après le nœud',
  ...
  changelog: {
    ...
    new_0_6_26_1: 'Cette version couvre les PR fusionnés après la 0.6.25, avec Ekko Agent, l affichage du context de Coding Agent, Journey, les baselines du chat de groupe, les correctifs desktop et la taille du payload MCP',
    new_0_6_26_2: 'Ekko Agent est maintenant disponible dans le chat avec la gestion du runtime context, les browser tools, des tool events plus stables et l historique tool-result restauré pour le calcul du context usage',
    new_0_6_26_3: 'Les sessions Coding Agent affichent plus clairement les réglages reasoning et context, persistent API mode, évitent le mauvais usage du Codex app-server mode et limitent les sauts de context usage après les tours de suivi',
    new_0_6_26_4: 'Journey ajoute une vue learning graph avec des interactions plus soignées, tandis que le chat de groupe reçoit une baseline coverage pour la nouvelle ligne de fonctionnalités',
    new_0_6_26_5: 'La fiabilité desktop progresse avec des correctifs de login persistence et reset flow, le fallback de port dynamique du Windows worker, la gestion stdout/stderr EPIPE et un packaging isolé du firmware ESP32-C3 v1',
    new_0_6_26_6: 'Hermes Studio MCP renvoie maintenant par défaut un résumé compact de available-models, avec filtres et option full-detail explicite pour éviter de saturer l agent context avec trop de données',
