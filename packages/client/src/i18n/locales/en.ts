export default {
  ...
  changelog: {
    ...
    new_0_6_39_1: 'Ekko now initializes explicitly, runs bounded Node.js or Python code behind tool approvals, and can pause foreground runs for text or multiple-choice clarification before resuming (#2362, #2368)',
    ...
    new_0_6_39_4: 'Coding Agent prompts now travel through stdin, preserving multiline and image input while avoiding Windows command-length limits. Group Chat also no longer interrupts active Codex, Claude, or Ekko work after a fixed two minutes (#2354, #2357)',
    ...
    new_0_6_36_6: 'Workspace diffs from Codex, Claude, and Ekko now appear only on the exact Assistant message that produced them across live, restored, and group chats (#2264)',
    ...
    new_0_6_35_1: 'Ekko persistent memory is now enabled in production, with profile-scoped SQLite storage, a 4,000-token automatic recall budget, improved full-store relevance search, and safer model request handling (#2246)',
    ...
    new_0_6_34_5: 'Coding Agent runs preserve image inputs across Codex, Claude, and scoped provider protocols, truncate oversized Responses tool outputs, and isolate runtime configuration and late events by session (#2222, #2232, #2238)',
    ...
    new_0_6_32_3: 'Workflow preserves each Hermes Bridge run source across handoff and reattachment, while scoped Codex and Claude nodes can use launcher-supported protocols for valid API-key targets (#2137, #2154)',
