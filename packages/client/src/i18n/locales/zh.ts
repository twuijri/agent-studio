export default {
  ...
  workflow: {
    ...
    evidence: {
      ...
      checkedData: '检查的数据', entireReplyText: '完整回复文本', errorText: '错误文本', jsonFieldValue: '单个 JSON 字段值', advancedPathValue: '高级路径的值', comparison: '比较方式', textToFind: '要查找的文字', expectedFieldValue: '期望的字段值', parsedBusinessDecision: '解析出的业务结论', failedGateLabel: '失败环节（failed_gate 字段的值）', failedGateValue: '失败环节（failed_gate 字段的值）：{gate}', literalTextContainsHelp: '这是字面文本检查：“{value}”可以出现在 JSON 的 key 或 value 中。',
