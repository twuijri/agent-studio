...
export default {
  ...
  ekkoConfig: {
    ...
    "managed": "Core Hub 관리",
    "custom": "사용자 지정",
    "managedHint": "Core Hub 관리 서버 정의는 자동으로 주입되며 프로필별로 편집, 삭제, 활성화 또는 비활성화할 수 있습니다.",
  ...
  browser: {
    title: '브라우저', settings: '브라우저 설정', desktopOnly: '내장 브라우저는 Core Hub Desktop에서만 사용할 수 있습니다.', newTab: '새 탭',
  ...
  agentManager: {
    ...
    refresh: '새로고침',
    hermesDescription: 'Hermes는 Core Hub가 관리하는 Runtime 또는 사용자가 설치한 로컬 CLI를 사용할 수 있습니다.',
    managedRuntime: 'Hermes Runtime',
    managedRuntimeHint: 'Core Hub는 전체 Runtime 패키지만 설치하고 관리하며 Hermes CLI를 별도로 설치하지 않습니다.',
    studioManaged: 'Core Hub 관리',
    ...
    inUse: '사용 중',
    builtIn: '내장',
    ekkoDescription: 'Ekko는 Core Hub에 포함되어 있어 별도로 설치, 업데이트 또는 삭제할 필요가 없습니다.',
    version: '버전',
    codingAgentDescription: 'Core Hub에서 설치, 업데이트 확인 및 삭제할 수 있습니다.',
    ...
    legacyDataMigrationWarning: '확인 후 Core Hub가 다시 시작되며, 로컬 서비스 시작 전에 같은 이름의 파일을 이전 복사본으로 덮어씁니다. 현재 디렉터리에만 있는 파일은 유지됩니다. 이 질문은 한 번만 표시됩니다.',
    legacyDataMigrationPositive: '이전 데이터 이전',
    legacyDataMigrationNegative: '이전하지 않음',
    legacyDataMigrationSuccess: '이전이 예약되었습니다. Core Hub가 다시 시작되고 로컬 서비스 시작 전에 데이터를 이전합니다.',
  ...
  runtimeVersions: {
    ...
    dataDirectoryEnvRestart: '변경 후 Core Hub를 완전히 종료했다가 다시 여세요. Windows 또는 Linux 데스크톱 앱에서 이전 값이 계속 보이면 로그아웃한 뒤 다시 로그인하세요.',
    cliUpdateDescription: 'Hermes Runtime 0.19.1 이상에서는 다음 명령으로 내장 Hermes Agent만 업데이트할 수 있습니다. Core Hub 데스크톱 앱이나 Web UI는 업데이트되지 않습니다. 실행 전에 Core Hub를 완전히 종료하세요:',
    webUiTitle: 'Web UI',
    platform: '플랫폼',
    ...
    runtimeDirectorySaved: 'Runtime 마이그레이션을 예약했습니다. Core Hub를 다시 시작하면 실행됩니다.',
    runtimeMigrationPending: '마이그레이션 예정 위치: {directory}',
    runtimeMigrationFailed: '저장소 마이그레이션에 실패하여 이전 저장소를 계속 사용합니다',
    runtimeActivationFailed: '선택한 Runtime을 시작할 수 없어 Core Hub가 설치된 다른 버전을 사용했습니다',
    active: '활성',
    installed: '설치됨',
    download: '다운로드',
    ...
    activateSuccess: '버전 선택이 저장되었습니다. Core Hub를 다시 시작하면 적용됩니다.',
  ...
  connections: {
    ...
    app: {
      ...
      viewDownload: '다운로드',
      viewMessages: '메시지 전송',
      downloadTitle: '휴대폰에서 Core Hub 사용하기',
      downloadDescription: '현재 비공개 베타 테스트 중이며 가입하려면 등록 코드가 필요합니다. 일부 등록 코드는 소셜 미디어를 통해 공개할 예정입니다.',
      downloadPaidDescription: 'Core Hub App이 정식 출시되었으며 현재 유료로 제공됩니다. 이용하려면 요금 및 구매 페이지에서 적합한 플랜을 선택하고 구매를 완료해 주세요.',
      ...
      accessFailures: {
        ...
        cloudSubscriptionRequired: '이 계정에는 Core Hub Cloud 이용 권한이 없습니다',
  ...
  appConnections: {
    ...
    notLinked: '먼저 이 컴퓨터를 연결하세요(기기 → 이 컴퓨터).',
    empty: '이 컴퓨터에서 MCP 서버가 있는 앱을 찾지 못했습니다.',
    emptyHint: '앱은 Claude Desktop, Claude Code, Codex, Cursor 또는 Windsurf에 MCP 서버를 등록합니다. 거기에서 앱의 AI 통합을 설치한 뒤 다시 검색하세요.',
    deviceConnected: '컴퓨터 연결됨',
    deviceNotConnected: '컴퓨터 연결되지 않음',
    shared: '{name}이(가) 서버와 공유되었습니다',
    unshared: '{name} 공유가 해제되었습니다',
    saveFailed: '공유 앱 업데이트 실패',
  ...
  chat: {
    ...
    pushEnabled: '푸시 사용',
    pushNotConfigured: '활성화하기 전에 Core Hub의 “기기 연결”에서 메시지 푸시를 설정하세요.',
    hideToolCalls: '도구 호출 숨기기',
    messageQueue: '메시지 대기열',
    removeQueuedMessage: '대기열 메시지 제거',
    steerQueuedMessage: '진행 중인 실행에 보내기',
    steerQueuedMessageLabel: '유도',
    ...
    batchArchive: '선택한 세션 {count}개 보관',
    batchUnarchive: '선택한 세션 {count}개 보관 해제',
    batchArchiveSuccess: '세션 {count}개를 보관했습니다',
    batchArchivePartial: '세션 {failed}개를 보관하지 못했습니다',
    batchArchiveFailed: '일괄 보관에 실패했습니다',
    batchUnarchiveSuccess: '세션 {count}개의 보관을 해제했습니다',
    batchUnarchivePartial: '세션 {failed}개의 보관을 해제하지 못했습니다',
    batchUnarchiveFailed: '일괄 보관 해제에 실패했습니다',
  ...
  models: {
    ...
    auxiliaryProviderStudioDefault: 'Core Hub 기본값',
    imageProvidersHermes: 'Hermes 지원',
    imageProvidersCustom: '사용자 지정 제공업체',
    imageProvidersHint: 'Hermes 제공업체는 자체 이미지 연동을 사용하며 설정이 필요합니다. 사용자 지정 제공업체는 Core Hub 이미지 API를 지원해야 합니다. 목록에 표시되어도 바로 사용할 수 있다는 뜻은 아닙니다.',
    ...
    auxiliaryTaskImageGeneration: 'Core Hub 이미지 생성',
    auxiliaryTaskImageEdit: 'Core Hub 이미지 투 이미지',
  ...
  profiles: {
    ...
    cloneFromProfile: '기존 프로필에서 복제',
    cloneSource: '복제 원본',
    cloneSourcePlaceholder: '복제할 프로필 선택',
    cloneSourceCurrent: '{name} (현재)',
  ...
  settings: {
    ...
    display: {
      ...
      busyInputModeHint: '에이전트가 작업 중일 때 보낸 메시지의 처리 방식: 차례를 기다리기, 실행을 멈추지 않고 유도하기, 현재 턴을 중단하고 시작하기.',
      busyInputModeQueue: '대기열에 넣기',
      busyInputModeSteer: '실행 유도',
      busyInputModeInterrupt: '중단하고 시작',
      theme: '테마',
    ...
    voice: {
      ...
      hermesConfigDescription: 'Hermes 프로필의 기존 네이티브 제공자는 유지되며 Web UI 제공자는 Core Hub로 표시됩니다.',
      ...
      useHermesStudioProvider: 'Core Hub 제공자 사용',
      ...
      groqCatalogHint: 'Groq API 키를 입력하세요. 아래 모델과 음성은 문서화된 Orpheus 목록을 사용합니다.',
      groqVoiceHint: '영어 또는 사우디 아랍어 모델과 해당 음성을 선택하세요. 긴 텍스트는 최대 200자씩 나누어 요청한 뒤 WAV 오디오로 합칩니다.',
