export default {
  ...
  workflow: {
    ...
    evidence: {
      ...
      summaryTitle: '실행 결과', blockedAt: '{gate}에서 차단됨', actualPath: '실제 경로', noActualPath: '선택된 경로 없음', pathChecks: '경로 판단', selectedPaths: '사용된 경로', otherPaths: '기타 판단 및 이벤트', selectedCount: '{count}개 사용', otherCount: '{count}개 미사용', eventCount: '이벤트 {count}개', showOtherPaths: '기타 세부 정보 보기 ({count})', hideOtherPaths: '기타 세부 정보 숨기기',
      ...
      checkedData: '검사한 데이터', entireReplyText: '전체 응답 텍스트', errorText: '오류 텍스트', jsonFieldValue: 'JSON 필드 하나의 값', advancedPathValue: '고급 경로 값', comparison: '비교 방법', textToFind: '찾을 텍스트', expectedFieldValue: '예상 필드 값', parsedBusinessDecision: '파싱한 비즈니스 결론', failedGateLabel: '실패 단계(failed_gate 필드 값)', failedGateValue: '실패 단계(failed_gate 필드 값): {gate}', literalTextContainsHelp: '문자 그대로 텍스트를 검사합니다. “{value}”는 JSON key 또는 value 어느 쪽에 있어도 일치합니다.',
  ...
  changelog: {
    ...
    new_0_6_26_1: '이번 릴리스는 0.6.25 이후 병합된 PR을 반영하며 Ekko Agent, Coding Agent context 표시, Journey, 그룹 채팅 baseline, 데스크톱 수정, MCP payload size에 중점을 둡니다',
    new_0_6_26_2: 'Ekko Agent는 이제 채팅에서 사용할 수 있으며 runtime context 처리, browser tools, 더 안정적인 tool event, context usage 계산을 위한 tool-result history 복원을 지원합니다',
    new_0_6_26_3: 'Coding Agent session은 reasoning과 context 설정을 더 명확히 보여 주고 API mode를 유지하며 Codex app-server mode 오용을 피하고 follow-up turn 뒤 context usage가 튀는 문제를 줄입니다',
    new_0_6_26_4: 'Journey에는 learning graph view와 다듬어진 graph interaction이 추가되었고, 그룹 채팅에는 새 기능 라인의 baseline coverage가 추가되었습니다',
    new_0_6_26_5: '데스크톱 안정성이 개선되어 login persistence와 reset flow, Windows worker dynamic port fallback, stdout/stderr EPIPE handling, 분리된 ESP32-C3 v1 firmware packaging을 보강했습니다',
    new_0_6_26_6: 'Hermes Studio MCP는 이제 available-models를 기본적으로 compact summary로 반환하며 filter와 명시적인 full-detail option으로 agent context에 많은 모델 데이터가 들어가는 일을 피합니다',
