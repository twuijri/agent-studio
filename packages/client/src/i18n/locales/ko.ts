...
export default {
  ...
  browser: {
    title: '브라우저', settings: '브라우저 설정', desktopOnly: '내장 브라우저는 Agent Studio Desktop에서만 사용할 수 있습니다.', newTab: '새 탭',
  ...
  runtimeVersions: {
    ...
    dataDirectoryEnvRestart: '변경 후 Agent Studio를 완전히 종료했다가 다시 여세요. Windows 또는 Linux 데스크톱 앱에서 이전 값이 계속 보이면 로그아웃한 뒤 다시 로그인하세요.',
    cliUpdateDescription: 'Hermes Runtime 0.19.1 이상에서는 다음 명령으로 내장 Hermes Agent만 업데이트할 수 있습니다. Agent Studio 데스크톱 앱이나 Web UI는 업데이트되지 않습니다. 실행 전에 Agent Studio를 완전히 종료하세요:',
    webUiTitle: 'Web UI',
    platform: '플랫폼',
    ...
    runtimeDirectorySaved: 'Runtime 마이그레이션을 예약했습니다. Agent Studio를 다시 시작하면 실행됩니다.',
    runtimeMigrationPending: '마이그레이션 예정 위치: {directory}',
    runtimeMigrationFailed: '저장소 마이그레이션에 실패하여 이전 저장소를 계속 사용합니다',
    runtimeActivationFailed: '선택한 Runtime을 시작할 수 없어 Agent Studio가 설치된 다른 버전을 사용했습니다',
    active: '활성',
    installed: '설치됨',
    download: '다운로드',
    ...
    activateSuccess: '버전 선택이 저장되었습니다. Agent Studio를 다시 시작하면 적용됩니다.',
  ...
  connections: {
    ...
    app: {
      ...
      viewDownload: '다운로드',
      viewMessages: '메시지 전송',
      downloadTitle: '휴대폰에서 Agent Studio 사용하기',
      downloadDescription: '현재 비공개 베타 테스트 중이며 가입하려면 등록 코드가 필요합니다. 일부 등록 코드는 소셜 미디어를 통해 공개할 예정입니다.',
      downloadPaidDescription: 'Agent Studio App이 정식 출시되었으며 현재 유료로 제공됩니다. 이용하려면 요금 및 구매 페이지에서 적합한 플랜을 선택하고 구매를 완료해 주세요.',
      ...
      accessFailures: {
        ...
        cloudSubscriptionRequired: '이 계정에는 Agent Studio Cloud 이용 권한이 없습니다',
  ...
  chat: {
    ...
    pushEnabled: '푸시 사용',
    pushNotConfigured: '활성화하기 전에 Agent Studio의 “기기 연결”에서 메시지 푸시를 설정하세요.',
  ...
  models: {
    ...
    imageProvidersHint: 'Hermes 제공업체는 자체 이미지 연동을 사용하며 설정이 필요합니다. 사용자 지정 제공업체는 Studio 이미지 API를 지원해야 합니다. 목록에 표시되어도 바로 사용할 수 있다는 뜻은 아닙니다.',
  ...
  settings: {
    ...
    voice: {
      ...
      hermesConfigDescription: 'Hermes 프로필의 기존 네이티브 제공자는 유지되며 Web UI 제공자는 Agent Studio로 표시됩니다.',
      ...
      useHermesStudioProvider: 'Agent Studio 제공자 사용',
