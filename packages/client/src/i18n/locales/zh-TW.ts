export default {
  ...
  changelog: {
    ...
    new_0_6_39_1: 'Ekko 新增明確初始化、受控的 Node.js/Python 程式碼執行與工具授權，並可在前景執行中透過既有互動卡片提出文字或多選澄清問題，取得回覆後繼續（#2362、#2368）',
    ...
    new_0_6_39_4: 'Coding Agent 提示詞改由 stdin 傳送，保留多行與圖片輸入並避開 Windows 命令長度限制；群聊也不再以固定兩分鐘上限中斷仍在工作的 Codex、Claude 或 Ekko（#2354、#2357）',
    ...
    new_0_6_36_6: 'Codex、Claude 和 Ekko 產生的 workspace diff 現在只會顯示在對應的助理訊息中，並統一適用於即時、還原和群聊場景（#2264）',
    ...
    new_0_6_35_1: 'Ekko 持久記憶現已在正式環境啟用，支援依 Profile 隔離的 SQLite 儲存、4000 Token 自動召回預算、更完善的全庫相關性檢索，以及更安全的模型請求處理（#2246）',
    ...
    new_0_6_34_5: 'Coding Agent 可在 Codex、Claude 和範圍限定的 Provider 協定間完整保留圖片輸入，自動截斷 Responses 過大的工具輸出，並依工作階段隔離執行設定和延遲事件（#2222、#2232、#2238）',
    ...
    new_0_6_32_3: 'Workflow 在 handoff 和重新連線過程中會保留每次 Hermes Bridge 執行的來源；Scoped Codex 和 Claude 節點也可為有效 API Key 目標使用 launcher 支援的協定（#2137、#2154）',
