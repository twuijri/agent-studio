export default {
  ...
  groupChat: {
    ...
    completeHistory: 'グループチャットの全履歴',
    readOnlyHistory: '読み取り専用履歴',
    loadingCompleteHistory: '全履歴を読み込み中...',
    completeHistoryLoadFailed: 'グループチャットの全履歴を読み込めませんでした。',
    loadingEarlierHistory: '以前のメッセージを読み込み中…',
    earlierHistoryLoadFailed: '以前のメッセージを読み込めませんでした。',
    retryEarlierHistory: '以前のメッセージを再読み込み',
    earliestHistoryReached: '最初のメッセージに到達しました。',
    backToRoom: 'ルームに戻る',
    enterCode: '招待コードを入力',
    yourName: 'あなたの名前',
  ...
  changelog: {
    ...
    new_0_6_40_1: 'このリリースには v0.6.39 以降にマージされた全 40 PR が含まれ、安全なリモートグループチャット、ローカル音声認識、Workflow スケジュール、グローバル承認、Chat Run Webhook、MCU、Coding Agent の信頼性を改善します',
    new_0_6_40_2: 'グループチャットに安全な共有ルームとリモート Agent を追加し、Agent の引き継ぎ、プレゼンス、失効処理を安定化しました。オフライン所有者のアバターを保持し、クリップボード API が使えない場合はルームリンクを手動コピーできます（#2408、#2410、#2411、#2415、#2423）',
    new_0_6_40_3: 'リモート Agent はファイル転送とクラウド workspace へのアップロードに対応しました。所有者以外の要求には workspace、機密情報、個人的な記憶の境界を厳格に適用し、所有権と承認ルーティングも修正しました（#2413、#2430、#2433、#2435）',
    new_0_6_40_4: '旧 Agent Link の移行を停止し、折りたたみ可能なローカル／リモートルーム一覧、リモートルームのローカル別名、共有ページへの移動、退出時の完全なクリーンアップを追加しました（#2424、#2459）',
    new_0_6_40_5: '履歴の自動削除を廃止し、mention を含むツール結果を上限付きメッセージ ID で安全に保存します。孤立ツール、承認の整合性とルーティング、token 集計によるイベントループ停止も修正しました（#2390、#2420、#2426、#2427、#2429、#2445）',
    new_0_6_40_6: '最近のセッションとグループチャットのアクティビティ表示を迅速化し、ツールパネルの展開に滑らかなアニメーションを追加しました（#2389、#2414）',
    new_0_6_40_7: 'グローバル承認は既存の確認要求を引き継ぎ、より明確なコマンドプレビューを表示し、保留中の操作をシステム通知で知らせます（#2406、#2425、#2432）',
    new_0_6_40_8: 'sherpa-onnx による中国語・英語対応のローカルストリーミング STT を追加しました。末尾の無音で最後の単語を保持し、macOS／Windows の Electron 互換音声バッファを使用します（#2447、#2448、#2461）',
    new_0_6_40_9: 'Workflow に永続的なスケジュール実行と、作成、編集、有効化、無効化、削除を行える管理 UI を追加しました（#2434、#2441）',
    new_0_6_40_10: 'Chat Run ライフサイクル Webhook を追加し、未使用の旧エンドポイントを削除しました。サーバーには軽量な生存確認エンドポイントも追加されています（#2422、#2452、#2453）',
    new_0_6_40_11: 'MCU で Ekko と Hermes Agent のランタイムを選択できるようになり、Global Agent のアバターと MCU 状態も正確に表示されます（#2438、#2442）',
    new_0_6_40_12: 'Coding Agent に実行 ID を必須化し、Windows の実行永続化分離、MCP タイムアウト、モデル間 reasoning 再生、コンテキスト使用量推定、重複する Studio ガイダンスを修正しました（#2387、#2392、#2395、#2396、#2398、#2416）',
    new_0_6_40_13: 'MoA プリセットでモデルごとの推論強度を設定でき、画像生成では不要なリモート保持を避けるため Responses ストレージを無効化しました（#2439、#2449）',
    new_0_6_39_1: 'Ekko は明示的な初期化、承認で保護された制限付き Node.js/Python コード実行、前景実行を一時停止してテキストまたは選択式の確認を受け取り再開する機能に対応しました（#2362、#2368）',
    ...
    new_0_6_39_4: 'Coding Agent のプロンプトは stdin 経由になり、複数行や画像入力を保ちながら Windows のコマンド長制限を回避します。Group Chat も稼働中の Codex、Claude、Ekko を固定 2 分で中断しません（#2354、#2357）',
    ...
    new_0_6_36_6: 'Codex、Claude、Ekko の workspace diff は、ライブ、復元、グループチャットのすべてで生成元の正確なアシスタントメッセージにだけ表示されます（#2264）',
    ...
    new_0_6_35_1: 'Ekko の永続メモリを本番環境で有効化しました。プロファイル単位で分離された SQLite ストレージ、4,000 トークンの自動想起予算、メモリ全体の関連性検索の改善、より安全なモデルリクエスト処理を提供します（#2246）',
    ...
    new_0_6_34_5: 'Coding Agent は Codex、Claude、スコープ付き Provider プロトコル間で画像入力を維持し、Responses の過大なツール出力を切り詰め、実行設定と遅延イベントをセッション単位で分離します（#2222、#2232、#2238）',
    ...
    new_0_6_32_3: 'Workflow はハンドオフや再接続をまたいで各 Hermes Bridge 実行元を保持し、スコープ付き Codex / Claude ノードは有効な API Key ターゲットに launcher 対応プロトコルを利用できます（#2137、#2154）',
