...
export default {
  ...
  ekkoConfig: {
    ...
    "managed": "Core Hub 管理",
    "custom": "カスタム",
    "managedHint": "Core Hub 管理のサーバー定義は自動的に注入され、プロファイルごとに編集、削除、有効化、無効化できます。",
  ...
  browser: {
    title: 'ブラウザー', settings: 'ブラウザー設定', desktopOnly: '内蔵ブラウザーは Core Hub Desktop でのみ利用できます。', newTab: '新しいタブ',
  ...
  agentManager: {
    ...
    refresh: '更新',
    hermesDescription: 'Hermes は、Core Hub 管理の Runtime またはユーザーがインストールしたローカル CLI を使用できます。',
    managedRuntime: 'Hermes Runtime',
    managedRuntimeHint: 'Core Hub は完全な Runtime パッケージのみをインストール・管理し、Hermes CLI を個別にはインストールしません。',
    studioManaged: 'Core Hub 管理',
    ...
    inUse: '使用中',
    builtIn: '内蔵',
    ekkoDescription: 'Ekko は Core Hub に同梱されているため、個別のインストール、更新、削除は不要です。',
    version: 'バージョン',
    codingAgentDescription: 'Core Hub からインストール、更新確認、削除ができます。',
    ...
    legacyDataMigrationWarning: '確認後に Core Hub が再起動し、ローカルサービスの開始前に同名ファイルを以前のデータで上書きします。現在のディレクトリにのみあるファイルは保持されます。この確認は一度だけ表示されます。',
    legacyDataMigrationPositive: '以前のデータを移行',
    legacyDataMigrationNegative: '移行しない',
    legacyDataMigrationSuccess: '移行を予約しました。Core Hub を再起動し、ローカルサービスの開始前にデータを移行します。',
  ...
  runtimeVersions: {
    ...
    dataDirectoryEnvRestart: '変更後は Core Hub を完全に終了して再度開いてください。Windows または Linux のデスクトップアプリで古い値が残る場合は、サインアウトして再度サインインしてください。',
    cliUpdateDescription: 'Hermes Runtime 0.19.1 以降では、次のコマンドで内蔵 Hermes Agent のみを更新できます。Core Hub デスクトップアプリや Web UI は更新されません。実行前に Core Hub を完全に終了してください：',
    webUiTitle: 'Web UI',
    platform: 'プラットフォーム',
    ...
    runtimeDirectorySaved: 'Runtime の移行を予約しました。Core Hub を再起動すると実行されます。',
    runtimeMigrationPending: '移行先：{directory}',
    runtimeMigrationFailed: '保存先の移行に失敗したため、以前の保存先を引き続き使用します',
    runtimeActivationFailed: '選択した Runtime を起動できなかったため、Core Hub は別のインストール済みバージョンを使用しています',
    active: '有効',
    installed: 'インストール済み',
    download: 'ダウンロード',
    ...
    activateSuccess: 'バージョン選択を保存しました。Core Hub を再起動すると反映されます。',
  ...
  connections: {
    ...
    app: {
      ...
      downloadTitle: 'スマートフォンで Core Hub を使う',
      downloadDescription: '現在クローズドベータ中です。登録には登録コードが必要で、今後ソーシャルメディアで一部のコードを配布します。',
      downloadPaidDescription: 'Core Hub App は正式にリリースされ、現在は有料でご利用いただけます。ご利用になるには、料金・購入ページで最適なプランを選び、購入手続きを完了してください。',
      ...
      accessFailures: {
        ...
        cloudSubscriptionRequired: 'このアカウントには Core Hub Cloud の利用権限がありません',
  ...
  appConnections: {
    title: 'このコンピューターのアプリ',
    subtitle: 'このコンピューター上で MCP サーバーを提供するアプリ。共有すると、サーバー上の Hermes がこのコンピューターを通じて使用できます。',
    rescan: '再スキャン',
    desktopOnly: 'このサーバーにリンクされた Core Hub デスクトップアプリからこのページを開いてください。',
    notLinked: '先にこのコンピューターをリンクしてください（デバイス → このコンピューター）。',
    empty: 'このコンピューターに MCP サーバーを持つアプリは見つかりませんでした。',
    emptyHint: 'アプリは Claude Desktop、Claude Code、Codex、Cursor、Windsurf に MCP サーバーを登録します。そこでアプリの AI 連携をインストールしてから再スキャンしてください。',
    deviceConnected: 'コンピューター接続済み',
    deviceNotConnected: 'コンピューター未接続',
    shared: '{name} をサーバーと共有しました',
    unshared: '{name} の共有を解除しました',
    saveFailed: '共有アプリの更新に失敗しました',
  ...
  devices: {
    ...
    linked: {
      ...
      bindingFailed: '許可するプロファイルの更新に失敗しました',
    },
    inboundStatus: {
      none: '申請可能',
      pending: '保留中',
  ...
  chat: {
    ...
    pushNotConfigured: '有効にする前に、Core Hub の「デバイス接続」でメッセージプッシュを設定してください。',
  ...
  models: {
    ...
    auxiliaryProviderStudioDefault: 'Core Hubデフォルト',
    imageProvidersHermes: 'Hermes対応',
    imageProvidersCustom: 'カスタムプロバイダー',
    imageProvidersHint: 'Hermesのプロバイダーは画像連携機能を使用し、設定が必要です。カスタムプロバイダーはCore Hubの画像APIに対応する必要があります。一覧への表示は利用準備の完了を意味しません。',
    ...
    auxiliaryTaskImageGeneration: 'Core Hub画像生成',
    auxiliaryTaskImageEdit: 'Core Hub画像から画像',
  ...
  settings: {
    ...
    display: {
      ...
      connectionMode: 'アプリの接続',
      connectionModeHint: 'このデスクトップアプリが Hermes を実行する場所: このコンピューター上、または自分の Core Hub サーバーに接続。変更するとアプリが再起動します。',
      connectionModeLocal: 'このコンピューター上',
      connectionModeServer: '{url} に接続中',
      connectionModeChange: '変更...',
    },
    agent: {
      maxTurns: '最大ターン数',
    ...
    voice: {
      ...
      hermesConfigDescription: 'Hermes プロファイルの既存ネイティブプロバイダーは保持され、Web UI のプロバイダーは Core Hub として表示されます。',
      ...
      useHermesStudioProvider: 'Core Hub プロバイダーを使用',
      ...
      groqCatalogHint: 'Groq APIキーを入力してください。以下のモデルと音声は公式ドキュメントのOrpheusカタログに基づいています。',
      groqVoiceHint: '英語またはサウジアラビアのアラビア語モデルと、その音声を選択します。長いテキストは最大200文字のリクエストに分割され、WAV音声に結合されます。',
