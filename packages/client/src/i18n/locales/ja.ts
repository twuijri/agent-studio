...
export default {
  ...
  connections: {
    ...
    app: {
      ...
      harmonyPending: 'AppGallery 版を準備中です',
  ...
  chat: {
    ...
    batchArchive: '選択した{count}件のセッションをアーカイブ',
    batchUnarchive: '選択した{count}件のセッションをアーカイブ解除',
    batchArchiveSuccess: '{count}件のセッションをアーカイブしました',
    batchArchivePartial: '{failed}件のアーカイブに失敗しました',
    batchArchiveFailed: '一括アーカイブに失敗しました',
    batchUnarchiveSuccess: '{count}件のセッションをアーカイブ解除しました',
    batchUnarchivePartial: '{failed}件のアーカイブ解除に失敗しました',
    batchUnarchiveFailed: '一括アーカイブ解除に失敗しました',
  ...
  changelog: {
    new_0_6_47_1: 'このリリースには v0.6.46 以降にマージされた 13 件の PR がすべて含まれ、独立したソーシャルメッセージ配信、設定可能な画像モデル、より信頼性の高いチャット引き継ぎ、フロントエンドの改善を提供します',
    new_0_6_47_2: '新しい「ソーシャルメッセージ」ワークスペースでは、Telegram、Feishu、Weixin の独立接続、チャネルごとの通知言語、ローカライズされた連携・状態通知、セッションごとのメッセージ配信を管理できます (#2718)',
    new_0_6_47_3: '画像生成と画像編集では、補助モデルから専用のプロバイダーとモデルを選択できるようになりました。リクエストで明示した選択と既存のデフォルトは引き続き優先されます (#2691)',
    new_0_6_47_4: 'いずれかの Agent が実行中の場合、グループチャットのルームでは合成アバター全体が発光し、行番号や列番号付きのワークスペースリンクは正しいファイルのプレビューを開きます (#2701, #2702)',
    new_0_6_47_5: 'Gateway の承認は中断または回答された正確なリクエストを解決するようになり、App Relay は安全な条件付き変更のために If-Match を保持します (#2681, #2710)',
    new_0_6_47_6: 'Codex と Claude のプロキシは system 指示を 1 つの先頭メッセージに統合し、メッセージ順序を厳格に検証する vLLM 系プロバイダーとの互換性を回復しました (#2688, #2714)',
    new_0_6_47_7: 'HERMES_MAX_UPLOAD_SIZE でアップロード上限を設定でき、README に hermes-web-ui-mcp CLI エイリアスの説明が追加されました (#2689, #2704)',
    new_0_6_47_8: 'App チャットの再開では、実行、キュー、使用量、セッションのメタデータを更新しながら、変更のないキャッシュ済みメッセージページを再利用し、フォアグラウンド復帰時の転送量を削減します (#2719)',
    new_0_6_47_9: '#2706 の GLM-5.3 推論強度の正規化とセッション選択の変更は #2708 で完全にロールバックされ、修正の再検討中は v0.6.46 の動作が維持されます',
    new_0_6_46_1: '本リリースは v0.6.45 以降にマージされた全 13 件の PR を含み、セッションと履歴の操作、再利用可能なグループチャット Agent、Git 対応ワークスペース、App Relay ルーティング、再開可能な実行の信頼性を改善します',
    new_0_6_46_2: '最近のセッションを折りたためるようになり、カテゴリ状態も表示されます。履歴では完了したツール呼び出しがライブチャットと同じコンパクトな実行グループにまとめられます（#2642、#2697）',
    new_0_6_46_3: 'グループチャットに、選択と管理を分離した再利用可能な Agent プリセットを追加しました。重複名エラーのローカライズ、無効モデルの検証、アクティブ Agent の虹色グロー復元も含まれます（#2644、#2670、#2678）',
    new_0_6_46_4: 'Coding Agent キューで安全なモデルまたはツール境界に次のタスクを挿入でき、各セッションの推論強度が再読み込み、デバイス、実行タイプをまたいで保存・同期されます（#2667、#2671）',
    new_0_6_46_5: 'ワークスペースエクスプローラーが Git 状態に対応しました。ファイルツリーに変更が表示され、変更済みファイルの Diff を確認でき、プレビューと編集でもリポジトリのコンテキストが保たれます（#2674）',
    new_0_6_46_6: 'ブラウザ注釈は入力欄に一時配置せず、構造化されたスクリーンショットコンテキストとして直接送信され、個別チャットとグループチャットの両方で引き渡しが安定します（#2679）',
    new_0_6_46_7: 'App Relay はローカル開発中もデスクトップ Relay を維持し、自動・直接・Relay のルート選択に対応しました。個別チャットの再開スナップショットも制限され、長い会話を効率よく再接続できます（#2682、#2683、#2696）',
    new_0_6_46_8: 'グループチャットの実行停止時に保留中の Ekko 確認要求を決定的に完了し、停止したチェーンのハングや予期しない再開を防ぎます（#2694）',
    new_0_6_45_1: 'このリリースには v0.6.44 以降にマージされた全 16 件の PR が含まれ、推論と Tool の履歴、モデルとダウンロードの管理、チャットの継続性、Group Chat の下書き、全体的な信頼性を改善します',
    new_0_6_45_2: 'ライブ推論と実行中の Tool は安定した 1 行表示になり、完了した Tool は明確な状態とアニメーションを備えた Run カードにまとまります。再開後もカードを保持し、DeepSeek の Tool 継続では推論内容を再送し、Global Codex、Claude、Pi のモデル設定は引き続き CLI が管理します（#2662）',
    new_0_6_45_3: 'モデル画面に Fallback タブが追加され、新しいセッション向けに最大 10 組の予備 Provider とモデルを追加し、ドラッグまたはキーボードで並べ替え、削除、保存できます（#2627）',
    new_0_6_45_4: 'Studio はリリースマニフェストから管理対象 Runtime のバージョンを読み込み、Android、Google Play、Apple、HarmonyOS 用の固定 QR コードを備えたモバイルダウンロードハブを提供します（#2659）',
    new_0_6_45_5: 'Group Chat の下書きは Mention を含めてルームごとに保存され、ルーム移動、再読み込み、送信失敗後も保持され、非同期送信が成功した場合のみ消去されます（#2643）',
    new_0_6_45_6: 'チャット移動の信頼性が向上しました。デスクトップではセッション切替後に入力欄へフォーカスし、/new の履歴を表示し続け、タイトルとアウトラインは文字方向を自動判定し、古い読み込み結果が新規チャットを上書きしません（#2606、#2609、#2614、#2650）',
    new_0_6_45_7: 'Coding Agent とバックグラウンドチャットの継続性が向上しました。画像非対応のエンドポイントをテキスト専用として記憶し、Pi はターン間でネイティブセッションを再開し、コールバックは発生元のコンテキストを保持します（#2655、#2656、#2657）',
    new_0_6_45_8: '操作とエラー表示が明確になりました。推論強度にはドラッグ可能な 8 段階が表示され、スケジュールには明示的な Cron 入力が必要となり、サイズ超過のアップロードは実際の 413 理由を返します（#2604、#2605、#2647）',
    new_0_6_45_9: 'メンテナンスとして Studio の旧 Web サイトを削除し、Node 24 で Worker ごとにテスト状態を分離して、重複するリリース経路と不安定な CI を減らしました（#2622、#2654）',
    new_0_6_44_1: 'このリリースには v0.6.43 以降にマージされた全 13 件の PR が含まれ、Group Chat のライブ履歴と実行状況、App 接続の信頼性、Provider の使いやすさ、Agent Bridge のメッセージ永続化を改善します',
    new_0_6_44_2: 'Group Chat でルームをまたいだ実行中の Agent を表示し、より高い Tool パネルを会話本文の上に配置しました。アクティブルームのセクションを折りたたみ、完全な履歴をライブルーム内で直接読み込めます（#2572、#2573、#2584、#2594）',
    new_0_6_44_3: 'App 接続は VPN アダプターより物理 LAN アドレスを優先し、古い Relay ペアリングコードを正しく更新します。モバイルクライアントには軽量なプロフィール画像を提供します（#2591、#2592、#2598）',
    new_0_6_44_4: '公開 Web サイトに英語と中国語のプライバシーポリシーを追加し、Google ユーザーデータ、権限の取り消し、保存、共有、Limited Use 要件を説明します（#2590）',
    new_0_6_44_5: 'モデルと Provider の画面は折りたたんだ Provider グループを各セレクターで共有します。モデルカタログを提供しない接続可能なサービスも、誤った接続失敗ではなく明確な警告後に保存できます（#2585、#2587）',
    new_0_6_44_6: 'ファイルマネージャーは ..hidden のように複数のドットで始まる正当な名前を受け入れ、本当の親ディレクトリ移動は引き続きブロックします（#2586）',
    new_0_6_44_7: 'Web UI の会話に設定済みのフォールバック Provider チェーンが渡され、プライマリモデルのレート制限など対象となるエラー時に予備 Provider へ切り替えられます（#2599）',
    new_0_6_44_8: 'Studio と Hermes がセッションデータベースを共有する場合でも、同じユーザーメッセージを履歴へ二重保存しなくなりました。先行書き込みの失敗時はネイティブ永続化が補完し、メッセージを失いません（#2601）',
    new_0_6_43_1: 'このリリースには v0.6.42 以降にマージされた全 19 件の PR が含まれ、Pi と Coding Agent のネイティブコマンド、安全な App・デバイス接続、Group Chat の完全履歴、キャンセル可能なキュー、信頼性を改善します',
    new_0_6_43_2: 'Pi が管理対象 Coding Agent として Studio に追加され、Scoped/Global RPC セッション、テキストと推論のライブ配信、Studio の承認と確認、分離設定、遅延 MCP ツール、Provider ルーティング、ネイティブセッション継続に対応しました（#2528）',
    new_0_6_43_3: 'Codex、Claude、Pi のチャットは入力が CLI に届く前に /context、/compact、/usage、/status を処理します。/compact は各 Agent のネイティブ圧縮を使用し、安全でない Studio フォールバックを行わずにネイティブエラーを報告します（#2566）',
    new_0_6_43_4: 'デバイス接続に、安全な LAN・クラウド App ペアリング、デバイス紐付けトークン、クラウドアカウント単位の分離、利用資格チェック、Docker から到達可能なオリジン、失効状態、自動 Relay 復旧を追加しました（#2523、#2557、#2561）',
    new_0_6_43_5: 'App 接続ページに、同期された Cloudflare・GitHub ソース、ダウンロード QR コード、Android APK の直接操作を備えた Android、iOS、HarmonyOS 向けレスポンシブダウンロードハブを追加しました（#2575）',
    new_0_6_43_6: 'Group Chat に共有音声入力と Agent ごとの正式な FIFO 実行キューを追加しました。未実行のメッセージは安全にキャンセルでき、所有アカウントからデバイスをまたいで取り消せます（#2555、#2570）',
    new_0_6_43_7: '統合された履歴画面から Group Chat の完全履歴を利用できます。直近 500 件の範囲を超えて安定した 150 件単位のカーソルページを取得でき、スクロール位置、認可、ライブ境界検出も維持されます（#2565、#2568）',
    new_0_6_43_8: '実行中の Group Chat に上限付きでスクロール可能な Tool リストを表示し、Tool トレースを所有する Agent Run に保持します。送信 Tool 結果は保存データや Agent コンテキストを変えず、表示だけを切り詰めます（#2527、#2536、#2548）',
    new_0_6_43_9: 'Group Chat のローリング要約カーソル、ハンドオフ深度停止、自己への混在メンション、承認・確認の Runtime フォールバックを強化し、並行・Tool 多用・リモート会話を復旧可能にしました（#2511、#2519、#2525、#2569）',
    new_0_6_43_10: 'セッションサイドバーのグループ分けで最近・ピン留め済みセッションを正しいカテゴリに維持し、Workflow Relay を修正しました。Kanban タスク本文は自身の左書き・右書き方向に自動追従します（#2546、#2545）',
    new_0_6_42_1: 'このリリースには v0.6.41 以降にマージされた全 13 件の PR が含まれ、Group Chat の信頼性、App Relay、安全なキュー挿入、Coding Agent の保守、サブエージェントのルーティング、MiniMax 動画生成を改善します',
    new_0_6_42_2: 'Group Chat はストリーミング処理を軽減しながらリモート操作のルーティングを維持し、Agent メンションを確実に重複排除し、同じアカウントの複数タブ間でメンバーシップと入力状態を正しく保持します（#2473、#2495、#2498、#2500）',
    new_0_6_42_3: 'App クライアントは LAN で Studio に直接接続し、App Relay 経由でリアルタイム Group Chat を利用し、ネイティブ MP3 録音をローカル STT に送信できます（#2487、#2509）',
    new_0_6_42_4: 'ルーム所有者は Agent の自動引き継ぎ深度を有限または無制限に設定し、自動チェーンを停止し、追加作業が必要なときに永続的な継続を 1 回だけ許可できます（#2504）',
    new_0_6_42_5: 'Hermes と Ekko のキューメッセージに安全な「次に実行」操作が追加され、ランタイムが管理するモデルまたはツール境界でのみ実行中の作業を停止し、タブ間で状態を同期します（#2477）',
    new_0_6_42_6: 'Coding Agents は npm の更新を確認し、最新バージョンを表示してその場でインストールし、完了後に表示状態を更新できます（#2505）',
    new_0_6_42_7: '回復可能な Codex ネイティブストリームエラーで成功した実行が失敗扱いにならず、Claude の構造化 API エラーは確実に保存されて一度だけ確定します（#2471、#2481）',
    new_0_6_42_8: '補助モデルに Profile 全体のサブエージェントルーティングが追加され、Provider、モデル、推論強度を設定できます。上書きがない場合、委任された Agent はメインモデルに従います（#2515）',
    new_0_6_42_9: 'MiniMax の画像から動画への生成は Global と中国の公式 v1 エンドポイント、URL、base64、ローカル画像、タスクのポーリング、Profile API Key による自動ダウンロードに対応しました（#2512）',
    new_0_6_41_1: 'npm でインストールした Hermes Web UI のローカル STT が安定して動作するよう、macOS、Windows、Linux の sherpa-onnx ネイティブランタイムを Node.js ラッパーと同じバージョンに固定しました（#2464）',
  },
}
