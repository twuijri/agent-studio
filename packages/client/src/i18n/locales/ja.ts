...
export default {
  ...
  browser: {
    title: 'ブラウザー', settings: 'ブラウザー設定', desktopOnly: '内蔵ブラウザーは Agent Studio Desktop でのみ利用できます。', newTab: '新しいタブ',
  ...
  runtimeVersions: {
    ...
    dataDirectoryEnvRestart: '変更後は Agent Studio を完全に終了して再度開いてください。Windows または Linux のデスクトップアプリで古い値が残る場合は、サインアウトして再度サインインしてください。',
    cliUpdateDescription: 'Hermes Runtime 0.19.1 以降では、次のコマンドで内蔵 Hermes Agent のみを更新できます。Agent Studio デスクトップアプリや Web UI は更新されません。実行前に Agent Studio を完全に終了してください：',
    webUiTitle: 'Web UI',
    platform: 'プラットフォーム',
    ...
    runtimeDirectorySaved: 'Runtime の移行を予約しました。Agent Studio を再起動すると実行されます。',
    runtimeMigrationPending: '移行先：{directory}',
    runtimeMigrationFailed: '保存先の移行に失敗したため、以前の保存先を引き続き使用します',
    runtimeActivationFailed: '選択した Runtime を起動できなかったため、Agent Studio は別のインストール済みバージョンを使用しています',
    active: '有効',
    installed: 'インストール済み',
    download: 'ダウンロード',
    ...
    activateSuccess: 'バージョン選択を保存しました。Agent Studio を再起動すると反映されます。',
  ...
  connections: {
    ...
    app: {
      ...
      downloadTitle: 'スマートフォンで Agent Studio を使う',
      downloadDescription: '現在クローズドベータ中です。登録には登録コードが必要で、今後ソーシャルメディアで一部のコードを配布します。',
      downloadPaidDescription: 'Agent Studio App は正式にリリースされ、現在は有料でご利用いただけます。ご利用になるには、料金・購入ページで最適なプランを選び、購入手続きを完了してください。',
      ...
      accessFailures: {
        ...
        cloudSubscriptionRequired: 'このアカウントには Agent Studio Cloud の利用権限がありません',
  ...
  chat: {
    ...
    pushNotConfigured: '有効にする前に、Agent Studio の「デバイス接続」でメッセージプッシュを設定してください。',
  ...
  models: {
    ...
    imageProvidersHint: 'Hermesのプロバイダーは画像連携機能を使用し、設定が必要です。カスタムプロバイダーはStudioの画像APIに対応する必要があります。一覧への表示は利用準備の完了を意味しません。',
  ...
  settings: {
    ...
    voice: {
      ...
      hermesConfigDescription: 'Hermes プロファイルの既存ネイティブプロバイダーは保持され、Web UI のプロバイダーは Agent Studio として表示されます。',
      ...
      useHermesStudioProvider: 'Agent Studio プロバイダーを使用',
