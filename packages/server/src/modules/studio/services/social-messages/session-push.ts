...
const SESSION_PUSH_MESSAGES: Record<SocialMessageBindingLocale, Record<SessionPushEvent, string>> = {
  zh: {
    'run.completed': '{agent} 有一条已完成消息，请到 Core Hub 查看',
    'approval.requested': '{agent} 有一条待授权消息，请到 Core Hub 授权',
    'clarify.requested': '{agent} 有一条待回答消息，请到 Core Hub 回答',
  },
  'zh-TW': {
    'run.completed': '{agent} 有一則已完成訊息，請到 Core Hub 查看',
    'approval.requested': '{agent} 有一則待授權訊息，請到 Core Hub 授權',
    'clarify.requested': '{agent} 有一則待回答訊息，請到 Core Hub 回答',
  },
  en: {
    'run.completed': '{agent} has a completed message. Open Core Hub to view it.',
    'approval.requested': '{agent} has a message awaiting authorization. Open Core Hub to authorize it.',
    'clarify.requested': '{agent} has a message awaiting your response. Open Core Hub to answer it.',
  },
  ja: {
    'run.completed': '{agent} から完了済みのメッセージがあります。Core Hub で確認してください。',
    'approval.requested': '{agent} から承認待ちのメッセージがあります。Core Hub で承認してください。',
    'clarify.requested': '{agent} から回答待ちのメッセージがあります。Core Hub で回答してください。',
  },
  ko: {
    'run.completed': '{agent}에 완료된 메시지가 있습니다. Core Hub에서 확인해 주세요.',
    'approval.requested': '{agent}에 승인 대기 중인 메시지가 있습니다. Core Hub에서 승인해 주세요.',
    'clarify.requested': '{agent}에 답변 대기 중인 메시지가 있습니다. Core Hub에서 답변해 주세요.',
  },
  fr: {
    'run.completed': '{agent} a un message terminé. Consultez-le dans Core Hub.',
    'approval.requested': '{agent} a un message en attente d’autorisation. Ouvrez Core Hub pour l’autoriser.',
    'clarify.requested': '{agent} a un message en attente de réponse. Ouvrez Core Hub pour y répondre.',
  },
  es: {
    'run.completed': '{agent} tiene un mensaje completado. Ábrelo en Core Hub.',
    'approval.requested': '{agent} tiene un mensaje pendiente de autorización. Abre Core Hub para autorizarlo.',
    'clarify.requested': '{agent} tiene un mensaje pendiente de respuesta. Abre Core Hub para responderlo.',
  },
  de: {
    'run.completed': '{agent} hat eine abgeschlossene Nachricht. Öffne Core Hub, um sie anzusehen.',
    'approval.requested': '{agent} hat eine Nachricht, die auf Freigabe wartet. Öffne Core Hub, um sie freizugeben.',
    'clarify.requested': '{agent} hat eine Nachricht, die auf deine Antwort wartet. Öffne Core Hub, um zu antworten.',
  },
  pt: {
    'run.completed': '{agent} tem uma mensagem concluída. Abra o Core Hub para visualizá-la.',
    'approval.requested': '{agent} tem uma mensagem aguardando autorização. Abra o Core Hub para autorizá-la.',
    'clarify.requested': '{agent} tem uma mensagem aguardando resposta. Abra o Core Hub para respondê-la.',
  },
  ru: {
    'run.completed': 'У {agent} есть завершённое сообщение. Откройте Core Hub, чтобы посмотреть его.',
    'approval.requested': 'У {agent} есть сообщение, ожидающее разрешения. Откройте Core Hub, чтобы разрешить его.',
    'clarify.requested': 'У {agent} есть сообщение, ожидающее ответа. Откройте Core Hub, чтобы ответить.',
  },
  ar: {
    'run.completed': 'لدى {agent} رسالة مكتملة. افتح Core Hub لعرضها.',
    'approval.requested': 'لدى {agent} رسالة بانتظار التفويض. افتح Core Hub لتفويضها.',
    'clarify.requested': 'لدى {agent} رسالة بانتظار الإجابة. افتح Core Hub للإجابة عنها.',
  },
}
