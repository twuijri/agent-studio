...
import { getDeviceRelation } from '../../repositories/devices-store'
...
export interface LanPeerFilesystemDependencies {
  ...
  getTerminalConfig: () => { cwd?: string }
  validatePath: (path: string) => string
}

let filesystemDependencies: LanPeerFilesystemDependencies | null = null

export function configureLanPeerFilesystem(dependencies: LanPeerFilesystemDependencies): void {
  filesystemDependencies = dependencies
}

function filesystem(): LanPeerFilesystemDependencies {
  if (!filesystemDependencies) throw new Error('Studio LAN peer filesystem has not been configured')
  return filesystemDependencies
}

function getActiveProfileDir(): string {
  return filesystem().getActiveProfileDir()
}

function getTerminalConfig(): { cwd?: string } {
  return filesystem().getTerminalConfig()
}

export function validateLanPeerPath(path: string): string {
...
class LanPeerConnection {
  ...
  private execCommand(msg: PeerJsonMessage) {
    ...
    if (msg.cwd) {
      try {
        const resolved = validateLanPeerPath(msg.cwd)
  ...
  private downloadFile(msg: PeerJsonMessage) {
    ...
    let filePath: string
    try {
      filePath = validateLanPeerPath(msg.path)
    } catch (err: any) {
